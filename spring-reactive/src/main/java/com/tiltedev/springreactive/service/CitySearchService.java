  ...
  public Flux<CitySearchResponse> search(String query) {
    ...
    return httpClient
        ...
        .flatMap(
            entry ->
                countryApiService
                    .fetchByCode(entry.getCountry_code())
                    .map(
                        country ->
                            CitySearchResponse.builder()
                                ...
                                .flagUrl(
                                    country.getFlags() != null ? country.getFlags().getPng() : null)
                                .build())
                    .onErrorResume(
                        e -> {
                          log.warn(
                              "Could not enrich country {} for {}: {}",
                              entry.getCountry_code(),
                              entry.getName(),
                              e.getMessage());
                          return Mono.just(
                              CitySearchResponse.builder()
                                  ...
                                  .longitude(entry.getLongitude())
                                  .countryCode(
                                      entry.getCountry_code() != null
                                          ? entry.getCountry_code().toUpperCase()
                                          : null)

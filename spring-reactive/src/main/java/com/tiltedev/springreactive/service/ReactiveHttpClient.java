...
public class ReactiveHttpClient {
  ...
  /**
   * Reactor Context key {@link com.tiltedev.springreactive.controller.RequestLoggingFilter} writes
  ...
  public <T> Mono<T> get(
      WebClient client, String path, Class<T> responseType, Consumer<UriBuilder> uriQueryParams) {
  ...
  private static String resolveRequestId(ContextView ctx) {
    return ctx.<String>getOrEmpty(REQUEST_ID_CONTEXT_KEY)
        .orElseGet(ReactiveHttpClient::generateRequestId);
  }
        ...
        .timeout(
            ...
            Mono.error(
                new ApiTimeoutException(
                    label, "Request timed out after " + timeout.toMillis() + "ms")))
    ...
    return Flux.defer(
        () -> {
          ...
          return toResponseSpec
              ...
              .doOnComplete(
                  () -> log.debug("[{}] <- {} (flux) {} complete", requestId, method, path))
              .onErrorMap(
                  WebClientRequestException.class,
                  e -> {
                    log.error("Connection failure {} {}: {}", method, path, e.getMessage());
              ...
              .onErrorMap(
                  ...
                  e -> {
                    log.error(
                        "Incomplete/malformed response {} {}: {}", method, path, e.getMessage());
                    return new ApiIncompleteResponseException(label, e.getMessage(), e);
                  })
              .doOnError(
                  ApiException.class,
  ...
  /**
   * Exponential backoff over transient failures only. The retried source still carries its own
   * timeout, so every attempt gets the full per-attempt budget. Once the retries run out the
   * original exception is thrown rather than Reactor's wrapper, so callers and {@code
   * GlobalExceptionHandler} keep seeing the typed {@code ApiException}.
   */
  private Retry retrySpec(
      String method, String path, String requestId, BooleanSupplier stillRetryable) {
  ...
  /**
   * Only failures a later attempt could plausibly survive: the connection never landed, the call
   * timed out, or the far side reported a server-side problem. 4xx responses and malformed bodies
   * are deterministic - retrying them just burns the budget.
   */

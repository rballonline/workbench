...
No Vue Router. `useAppNav` reads `?page=welcome` (default) from the URL and updates it via `history.pushState`, which preserves the other query params — `?api=` in particular must survive navigation.
...
|---|---|
| `welcome` | (aggregates the destinations store) |
...
`GlobalExceptionHandler` on the backend answers *every* error with an RFC 7807 body, so a 5xx **without** a JSON body did not come from the app — it is the Vite dev proxy reporting that the backend is down. `apiFetch` remaps that case to `status: 0`, same as a fetch-level network failure.
